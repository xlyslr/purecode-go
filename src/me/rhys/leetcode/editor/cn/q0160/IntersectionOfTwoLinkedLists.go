...
//O(1) memory? Related Topics 哈希表 链表 双指针 👍 1543 👎 0

package q0160

import . "rhys.me/purecode-go/src/me/rhys/leetcode/editor/cn"
import . "rhys.me/purecode-go/src/me/rhys/leetcode/editor/cn/q0142"
...
func getIntersectionNode(headA, headB *ListNode) *ListNode {
	// 方法一：A+B 然后B+A 遍历寻找相同点
	return methodA(headA, headB)

	// 方法二：A+B 寻找环的起点，前提是A和B自身没有环
	//return methodB(headA, headB)
}

func methodB(headA, headB *ListNode) *ListNode {
	ab := headA
	for ; ; ab = ab.Next {
		if ab.Next == nil {
			ab.Next = headB
	...
	return DetectCycle(ab)
}

func methodA(headA, headB *ListNode) *ListNode {
	a := headA
	b := headB
	for a != b {
		if a == nil {
